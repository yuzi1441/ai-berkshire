...
保留以上所有分析正文、表格和结论，不改写、不压缩。完成正文后，在**报告最后**追加下表，供投资看板稳定读取；它只是对正文最终基本面结论的索引和一致性校验，不能替代分析，也不能写入技术面信号。

- 仅用于单一上市公司的完整基本面研究；多公司、行业或主题报告不要附此表。
- `数据截止日`是本报告基本面与估值判断的事实截止日，不能用报告撰写日代替。
- `基本面建议动作`只能填：`买入`、`分批买入`、`持有`、`观察`、`减仓/卖出`，必须与正文最终建议一致。
- 只有迁移旧报告时，若原文无法可靠提取动作，才允许填`待复核`；这类契约不会替代旧解析结论。
- 三档价格均为基本面估值/安全边际价格，不得填技术面价格；无可靠价格时填`未给出`，不得编造。
- 每个字段都必须填写。未知信息统一填`未给出`或`待复核`；价格不适用时填`未给出`。表格内容不得含竖线（`|`）。
