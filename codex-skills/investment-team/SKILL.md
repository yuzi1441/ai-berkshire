...
保留以上完整研究正文和所有既有表格。完成总结后，在**报告最后**追加下表，作为看板的机器可读索引；表内只能归纳正文已经得出的基本面结论，不能把技术面指标、技术面价格或自动交易指令写入其中。

- `数据截止日`必须是基本面与估值判断实际使用的事实截止日，不得拿报告撰写日替代。
- `基本面建议动作`仅限：`买入`、`分批买入`、`持有`、`观察`、`减仓/卖出`，并与第7部分的最终建议一致。
- 只有迁移旧报告时，若原文无法可靠提取动作，才允许填`待复核`；这类契约不会替代旧解析结论。
- 三档价格均为基本面估值/安全边际价格；没有可靠结论时填`未给出`，不要推测或沿用旧报告价格。
- 所有字段必须出现；未知值填`未给出`或`待复核`。表格单元格中不得使用竖线（`|`）。
