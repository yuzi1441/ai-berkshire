...
- C 级：聚合转载或未能确认的其他媒体，仅作为辅助，不计入评分。
- D 级：雪球、股吧、微博、自媒体等社区/传闻，仅作为辅助，不计入评分。
...
每只股票默认保留 8 条可评分候选，同时额外抓取最多 20 条 C/D 级辅助资讯。辅助池只
增加看板可见信息，不发送给模型，因此不会增加模型评分 token；可通过
`--auxiliary-news-limit` 调整。
