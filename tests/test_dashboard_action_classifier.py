...
class DashboardActionClassifierTests(unittest.TestCase):
    ...
    def test_dashboard_controls_filter_current_executability(self):
        ...
        self.assertIn("renderHumanReviewCompactCell(item, quote)", app)
        self.assertIn("humanReviewTaskCompactDateText", app)
        self.assertIn("humanReviewTaskDateText", app)
        self.assertIn("东财+巨潮核验", app)
        self.assertIn('"报告价格档 × 现价位置"', app)
        self.assertIn("renderZoneBar(item, quote)", app)
        self.assertIn("renderExecutionMainCell", app)
        self.assertIn("renderIdentityCell", app)
        self.assertIn("renderTechnicalDetail(item)", app)
        self.assertIn("renderDualModelCompactCell(item, mrcGeneratedAt)", app)
